...
def exit_leads_to(exits, direction):
    ...
    'Reception'
    """
    exitroom = exits[direction]
    return str(rooms[exitroom]["name"])
...
def print_menu(exits):
    ...
    ### In this for loop, direction is declared as the key, and next_room is declared as the value.
    ### The direction is the key in the dictionary, and the room is the value.
    ### It works the same if you replaced those words with key and value.
    for direction, next_room in exits.items():
        next_room = exit_leads_to(exits, direction)
        print_menu_line(direction, next_room)
    ### This takes the print_menu_line function and uses it to display the available rooms in a more visually appealing manner than just a dictionary print.

    # The for loop iterates over every entry in the dictionary and do what is in the loop
    # COMPLETE THIS PART:
...
def move(exits, direction):
    ...
    next_room_name = exits[direction] # = Parking

    next_room = rooms[next_room_name] # = room_parking
